<template>
  <div>
    <div class="orderHeader">
      <h1><span class="iconfont icon-jiantou2" style="font-size: 0.24rem" @click='backToCart'></span><em>订单信息</em></h1>
    </div>
    <div class="orderContent">
      <div class="order_content_title">
        <p @click="goAddress">{{addressStr}}</p>
        <span class="iconfont icon-jiantouxiangyou"></span>
      </div>
...
export default {
  ...
  methods: {
    getAddress () {
      var addressStr = JSON.parse(localStorage.getItem('userAddress'))
      if (addressStr) {
        this.addressStr = addressStr.user + '    ' + addressStr.phone + '     ' + addressStr.address
      }
    },
    backToCart () {
      window.history.go(-1)
    ...
    getOderList () {
      var orderListStr = localStorage.getItem('selectlist')
